WITH params AS (
  SELECT (SELECT MAX(date) FROM public.ads_spend)::date AS end_date,
         30::int AS n_days
),
bounds AS (
  SELECT
    (end_date - (n_days - 1))    AS last_start,   -- inclusive n_days
    end_date                     AS last_end,
    (end_date - (2*n_days - 1))  AS prev_start,   -- inclusive n_days
